  ...
  const ImageGallery: React.FC<ImageGalleryProps> = ({ imageBlurhash, image }) => {
    return (
      <div className="flex justify-center items-start h-150">
        ...
        <div className="relative grid grid-cols-2 gap-3 ml-4 w-2/3 h-full">
          {Array(4).fill().map((_, index) => (
            <div key={index} className="relative h-full w-full overflow-hidden rounded-2xl hover:scale-105">
              <BlurImage
